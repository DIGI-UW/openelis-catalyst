...
const canonicalJson = (value: unknown): string => {
  ...
  if (typeof value === "number") {
    if (!Number.isFinite(value)) {
      throw new TypeError("The editor snapshot contains a non-finite number.");
    }
    return JSON.stringify(value);
  }
  ...
  throw new TypeError("The editor snapshot contains a non-JSON value.");
};

const ROTATE_RIGHT = (value: number, count: number) =>
...
const sha256Hex = (source: string) => {
  ...
  for (let offset = 0; offset < bytes.length; offset += 64) {
    ...
    for (let index = 16; index < 64; index += 1) {
      ...
      const sigma0 =
        ROTATE_RIGHT(previous15, 7) ^
        ROTATE_RIGHT(previous15, 18) ^
        (previous15 >>> 3);
      const sigma1 =
        ROTATE_RIGHT(previous2, 17) ^
...
export const workbenchEditorDigest = (content: EditorContent) =>
